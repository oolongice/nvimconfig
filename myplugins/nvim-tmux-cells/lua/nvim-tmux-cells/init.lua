...
-- Usage (lazy example):
-- {
--   "yourname/nvim-tmux-cells",
--   config = function()
--     require("nvim-tmux-cells").setup({
--       target = "0:1.0",       -- your tmux target: session:window.pane (required)
--       send_enter = true,       -- press Enter after pasting into tmux
--       trim_leading_blank = true, -- trim leading blank lines in a cell
--     })
--   end,
-- }
--
-- Commands:
--  :TmuxSetTarget <target>   -- e.g. :TmuxSetTarget 0:1.0
--  :TmuxShowTarget           -- echo current target
--  :TmuxSendCell             -- send the current cell (surrounded by #%%) to tmux
--
-- Mappings (example):
--   vim.keymap.set("n", "<leader>tc", require("nvim-tmux-cells").send_current_cell, { desc = "Send cell to tmux" })
--   vim.keymap.set("n", "<leader>tt", function() require("nvim-tmux-cells").set_target("0:1.0") end, { desc = "Set tmux target" })
...
local function find_cell_bounds(bufnr, cursor_row)
	...
	-- if the current line itself is a marker, start after it
	...
	-- search downward for next marker
	local end_row = total
...
local function tmux_available()
	local ok = vim.fn.executable("tmux") == 1
	return ok
end
...
local function tmux_paste(text)
	...
	-- use a dedicated buffer name to avoid clobbering user's clipboard buffers
	local bufname = "nvim_tmux_cells"

	-- load text into a tmux buffer and paste to target
	local load_cmd = { "tmux", "load-buffer", "-b", bufname, tmp }
	local paste_cmd = { "tmux", "paste-buffer", "-t", target }
	...
	if M.config.send_enter then
		-- Send an Enter key to execute (useful when target pane runs ipython/python REPL)
